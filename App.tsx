...
const App: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'map' | 'strategic'>('grid');
  ...
  const SidebarItem = ({ icon, label, active, onClick }: any) => (
    <button
      ...
      className={`w-full flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3 sm:py-4 rounded-xl sm:rounded-2xl transition-all active:scale-[0.97] ${active ? 'bg-blue-600/10 text-blue-500 border border-blue-500/20 shadow-lg' : 'text-zinc-500 hover:bg-white/5 hover:text-zinc-300'}`}
    >
      <div className={`${active ? 'text-blue-500' : 'text-zinc-600'}`}>{icon}</div>
      <span className="text-[10px] sm:text-xs font-black uppercase tracking-[0.15em] sm:tracking-widest">{label}</span>
    </button>
  );

  const SidebarContent = () => (
    <>
      <div className="h-20 flex items-center px-6 border-b border-zinc-800/40 bg-black/20">
        <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center mr-4 shadow-[0_10px_30px_rgba(59,130,246,0.3)] shrink-0 transition-transform hover:rotate-3">
        ...
        <div className="flex flex-col">
          <span className="font-black text-xl tracking-tighter uppercase leading-none text-white">TrafficOS</span>
          <span className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.4em] mt-1.5">Uplink Beta</span>
        </div>
      </div>
      ...
      <div className="p-4 flex-1 space-y-2 overflow-y-auto custom-scroll">
        ...
        <SidebarItem
          ...
          label="Strategic"
          icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 21V3m9 6H3m18 6H3" /></svg>}
        />
        <SidebarItem
        ...
        <div className="pt-8 pb-3 px-5">
          <span className="text-[10px] font-black text-zinc-700 uppercase tracking-[0.2em] sm:tracking-widest">Network Analytics</span>
        </div>
        ...
        <div className="px-5 space-y-5">
          ...
          <div className="flex justify-between items-center text-[10px] sm:text-[11px]">
            ...
            <span className="font-mono text-zinc-500 font-bold">{stats.redLight}</span>
          </div>
          <div className="flex justify-between items-center text-[10px] sm:text-[11px]">
            <span className="text-zinc-600 font-bold uppercase tracking-wide">Speed Intel</span>
            <span className="font-mono text-zinc-500 font-bold">{stats.speed}</span>
          </div>
        </div>
      </div>

      <div className="p-6">
        <div className="glass rounded-[2rem] p-5 shadow-2xl relative overflow-hidden group">
          <div className="absolute inset-0 bg-blue-600/5 transition-opacity opacity-0 group-hover:opacity-100"></div>
          ...
          <div className="flex justify-between items-center text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-4 relative">
            <span>Last Uplink</span>
            <span className="font-mono text-zinc-300">{lastSync}</span>
          </div>
          <div className="w-full bg-zinc-800/50 h-1 rounded-full overflow-hidden relative">
            <div className="bg-blue-600 h-full transition-all ease-linear duration-1000 shadow-[0_0_10px_#3b82f6]" style={{ width: `${(countdown / REFRESH_INTERVAL_SECONDS) * 100}%` }}></div>
          </div>
        </div>
      </div>
    </>
  ...
  return (
    <div className="flex h-screen bg-[#09090b] text-zinc-100 overflow-hidden font-sans">
      ...
      <aside className={`
        fixed inset-y-0 left-0 w-72 bg-[#09090b]/95 backdrop-blur-xl border-r border-zinc-800/40 z-[100] transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] md:relative md:translate-x-0 md:flex md:flex-col
        ${isMobileMenuOpen ? 'translate-x-0 shadow-[40px_0_100px_rgba(0,0,0,0.8)]' : '-translate-x-full'}
      `}>
        <SidebarContent />
      </aside>

      <main className="flex-1 flex flex-col relative min-w-0 overflow-hidden bg-[#070708]">

        <header className="h-20 border-b border-zinc-800/40 bg-[#09090b]/80 backdrop-blur-3xl flex items-center justify-between px-4 sm:px-8 sticky top-0 z-[80]">
          <div className="flex items-center gap-2 sm:gap-4 flex-1">
            <button
              onClick={() => setIsMobileMenuOpen(true)}
              className="p-2 sm:p-3 -ml-2 sm:-ml-3 text-zinc-400 hover:text-white md:hidden active:scale-90 transition-transform"
            >
            ...
            <div className={`
             ...
             `}>
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                <svg className="h-4 w-4 text-zinc-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              </div>
              <input
                type="text"
                className="block w-full pl-11 pr-11 py-2.5 sm:py-3 border border-zinc-800/50 rounded-2xl leading-5 bg-zinc-900/60 text-zinc-200 placeholder-zinc-700 focus:outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/5 transition-all text-sm font-medium"
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            <div className="flex flex-col items-end shrink-0">
              <span className="text-[10px] font-black text-zinc-700 uppercase tracking-widest hidden xs:block mb-0.5">Network Coverage</span>
              <div className="flex items-center gap-2">
                <div className="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse hidden xs:block"></div>
                <span className="text-[10px] sm:text-xs font-mono text-blue-500 font-black">
                  {Math.round((stats.visible / stats.total) * 100)}% ACTIVE
                </span>
              </div>
            </div>
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-zinc-900/80 border border-zinc-800 flex items-center justify-center text-[10px] font-black text-zinc-600 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] group cursor-pointer hover:border-blue-500/50 hover:text-zinc-400 transition-all active:scale-95">
              UX
            </div>
          </div>
        </header>
        ...
        {viewMode === 'grid' ? (
          <div className="flex-1 flex flex-col min-h-0">
            <div className="px-4 sm:px-8 py-3 border-b border-zinc-800/30 bg-[#09090b]/40 backdrop-blur-xl relative overflow-hidden flex-shrink-0">
              <div className="flex items-center gap-2 overflow-x-auto custom-scroll no-scrollbar pb-1 mask-linear-fade">
                {regions.map(region => (
                  <button
                    key={region}
                    onClick={() => setSelectedRegion(region)}
                    className={`px-4 py-2 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] transition-all whitespace-nowrap active:scale-95 ${selectedRegion === region
                      ? 'bg-blue-600 text-white shadow-[0_8px_20px_rgba(59,130,246,0.3)]'
                      : 'bg-zinc-900/80 border border-zinc-800/60 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/80'
                      }`}
                  >
                    {region}
                  </button>
            ...
            <div className="flex-1 overflow-y-auto custom-scroll p-4 sm:p-8">
              {loading ? (
                <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 sm:gap-6">
                  {[...Array(10)].map((_, i) => (
                    <div key={i} className="bg-zinc-900/40 rounded-3xl border border-zinc-800/50 h-56 sm:h-72 animate-pulse"></div>
                  ))}
                </div>
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-10 animate-in fade-in zoom-in-95 duration-500">
                  ...
                  <h3 className="text-lg font-bold text-zinc-300 mb-2 uppercase tracking-tight">No surveillance nodes found</h3>
                  <p className="text-zinc-600 text-sm mb-6 max-w-xs">Try adjusting your search query or filter parameters to find specific units.</p>
                  <button
                    onClick={clearSearch}
                    className="px-8 py-3 bg-zinc-800 hover:bg-zinc-700 text-[10px] font-black uppercase tracking-widest text-zinc-300 rounded-2xl border border-zinc-700 transition-all active:scale-95 shadow-lg"
                  >
                    Reset Search Status
                  </button>
                </div>
              ) : (
                <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 3xl:grid-cols-6 gap-4 sm:gap-6">
                  ...
                  <div ref={observerTarget} className="h-24 w-full col-span-full flex items-center justify-center">
                    {visibleCount < filteredCameras.length && (
                      <div className="flex flex-col items-center gap-3">
                        <div className="w-8 h-8 border-2 border-blue-500/20 border-t-blue-500 rounded-full animate-spin"></div>
                        <span className="text-[9px] font-black text-zinc-700 uppercase tracking-widest">Loading More Intel</span>
                      </div>
                    )}
                  </div>
                </div>
