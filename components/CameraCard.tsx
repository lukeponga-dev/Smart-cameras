...
const CameraCard: React.FC<CameraCardProps> = React.memo(({ camera, isFavorite, onToggleFavorite, onViewLive }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-video bg-zinc-900 overflow-hidden">
        ...
        <div className={`absolute top-3 right-3 z-10 transition-opacity ${isFavorite ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
          <button
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-xl backdrop-blur-xl border border-white/10 shadow-2xl transition-all active:scale-90 ${isFavorite ? 'bg-amber-500 text-white shadow-amber-500/20' : 'bg-black/60 text-white hover:bg-black/80'
              }`}
          >
        ...
        {analysis && (
          <div className="absolute bottom-3 left-3 z-10">
            <div className={`px-2.5 py-1 rounded-lg shadow-2xl backdrop-blur-xl border border-white/10 flex items-center gap-2 ${analysis.level === 'heavy' ? 'bg-red-600/90' :
      ...
      {/* Content Section */}
      <div className="p-3 sm:p-4 flex flex-col gap-2 sm:gap-3 flex-1 relative overflow-hidden">
        <div className="absolute -right-4 -bottom-4 w-16 h-16 bg-zinc-800/10 rounded-full"></div>

        <div className="flex items-start justify-between gap-3 relative">
          <h3 className="font-bold text-[11px] sm:text-sm text-zinc-100 leading-tight line-clamp-2 pr-2" title={camera.name}>
            {camera.name}
          </h3>
          <div className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full mt-1 shrink-0 shadow-[0_0_8px_currentColor] ${statusColor} ${camera.status === 'Operational' ? 'animate-pulse' : ''}`} style={{ color: statusColor.replace('bg-', '') }}></div>
        </div>

        <div className="flex items-center gap-2 mt-auto relative">
          <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg text-[8px] sm:text-[9px] font-black bg-zinc-900 border border-zinc-800 text-zinc-500 uppercase tracking-widest">
            {camera.region}
          </span>
          {camera.direction !== 'N/A' && (
            <span className="text-[8px] sm:text-[9px] text-zinc-600 font-black uppercase tracking-tighter">{camera.direction} VECTOR</span>
          )}
        </div>
      </div>
    </div>
